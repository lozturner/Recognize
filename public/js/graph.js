...
function drawChart(event) {
	...
	newContent.id = "csv_buttons";
	newContent.innerHTML = "<ul><li><a href='" + event.target.id + ".csv' class='btn admin-link'>Download this chart data as CSV</a></li>" +
		"<li><a href='' class='btn admin-link'>Download all award data as CSV</a></li></ul>";  //ADD href
	...
	else if (event.target.id == "received")
	{
		...
		var receivedData = [];
		for(var i = 0; i < awardsTable.length; i++){
			var name = awardsTable[i].recipient_name;
		...
		for(var prop in receivedData){
			data.addRow([prop, receivedData[prop]]);
		}

		// Set chart options
		options = {
			'title':'Most Awarded Users',
			'width':700,
			'height':450,
			'backgroundColor':backgroundColor,
	...
	else if (event.target.id == "sent")
	{
		...
		var sentData = [];
		for(var i = 0; i < awardsTable.length; i++){
			var name = awardsTable[i].sender_name;
			if(!sentData[name]){
				sentData[name] = 1;
			} else {
				sentData[name]++;
		...
		data.addColumn('string', 'Name');
		data.addColumn('number', 'Awards');
		for(var prop in sentData){
			data.addRow([prop, sentData[prop]]);
		}
		
		// Set chart options
		options = {
			'title':'Number of awards given',
			'width':700,
			'height':450,
			'backgroundColor':backgroundColor,
			'sliceVisibilityThreshold': '.1'
